...
def _get_schema(hass: Any, user_input: list, default_dict: list) -> Any:
    ...
    return vol.Schema(
        {
            vol.Required(CONF_TEAM_ID, default=_get_default(CONF_TEAM_ID)): int,
            vol.Optional(CONF_NAME, default=_get_default(CONF_NAME)): str,
            vol.Optional(CONF_API_KEY, default=_get_default(CONF_API_KEY)): str,
        }
    )


@config_entries.HANDLERS.register(DOMAIN)
class PALLONEsensorFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """Config flow for PALLONE."""
    ...
    def async_get_options_flow(config_entry):
        return PALLONEsensorOptionsFlow(config_entry)


class PALLONEsensorOptionsFlow(config_entries.OptionsFlow):
    """Options flow for PALLONE."""
